...
export default function BillingPage() {
  ...
  const billingLabel = formatBillingStatusLabel(environmentAccess.entitlementStatus)
      ...
      {!accessError && accessState ? (
        <>
          ...
          <section className="rounded-xl border border-zinc-200 bg-white p-4">
            ...
            <Text className="mt-1 text-sm">Can create keys: {environmentAccess.canCreateKey ? 'Yes' : 'No'}</Text>
            ...
            {!environmentAccess.environmentId ? (
              <Text className="mt-1 text-sm text-zinc-600">No environment ID is provisioned for {selectedEnvironment} yet.</Text>
            ) : null}
          ...
          <section className="rounded-xl border border-zinc-200 bg-white p-4">
            ...
            <div className="mt-3">
              <BillingActions status={billingLabel} showUpgrade={!environmentAccess.canCreateKey} />
            </div>
          </section>
        </>
      ) : null}
