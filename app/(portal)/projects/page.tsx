...
export default function ProjectsPage() {
  ...
  const onCreate = async () => {
    ...
    setError(null)
    try {
      const created = await createProject(name.trim(), 'sandbox')
      setName('')
    ...
    <PageShell title="Projects" description="Create and manage your API projects.">
      <section className="rounded-xl border border-zinc-200 bg-white p-4">
        <div className="flex flex-wrap items-end gap-2">
          ...
          <Button color="dark/zinc" onClick={onCreate} disabled={busy || loadingProjects}>
            Create project
          </Button>
        </div>
          ...
          <TableBody>
            {projects.map((project) => (
              <TableRow key={project.id}>
                ...
                <TableCell>{project.environment ?? 'sandbox'}</TableCell>
            ...
            {!loadingProjects && projects.length === 0 ? (
              <TableRow>
                <TableCell colSpan={4}>No projects yet. Create one to get started.</TableCell>
              </TableRow>
            ) : null}
          </TableBody>
        </Table>
