...
export default function KeysPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const list = await portalApi.listApiKeys(selectedProjectId, environment)
      setKeys(list)
    } catch (err) {
  ...
  const onCreate = async () => {
    if (!selectedProjectId) {
      setError({
        status: 422,
        code: 'UNKNOWN',
        userMessage: 'No project selected. Choose a project from the header first.',
    ...
    try {
      const created = await portalApi.createApiKey(selectedProjectId, { name: name.trim(), environment })
  ...
  const onConfirmRevoke = async () => {
    if (!selectedProjectId || !revokeTarget) {
      return
    }
    ...
    try {
      await portalApi.revokeApiKey(selectedProjectId, revokeTarget.id, environment)
    ...
    <PageShell title="API Keys" description={`Manage ${environment} keys. Secret values are shown once.`}>
      {!selectedProjectId ? (
        <section className="rounded-xl border border-zinc-200 bg-white p-4 text-sm text-zinc-600">
          Select a project from the header to manage API keys.
        </section>
      ) : (
        <>
